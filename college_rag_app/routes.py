...
# Get the loggers that were configured in create_app()
...
def login():
    try:
        ...
        if username == Config.DEMO_USER and password == Config.DEMO_PASSWORD:
            ...
            security_logger.info(f"User '{username}' logged in successfully.")
    ...
    try:
        ...
        if username != current_user:
            security_logger.warning(
                f"Forbidden access attempt by '{current_user}' on user '{username}'."
            )
        ...
        app_logger.info(f"Session '{session_name}' created for user '{username}'.")
...
async def ask():
    ...
    try:
        ...
        if username != current_user:
            security_logger.warning(
                f"User '{current_user}' attempted to access session for user '{username}'."
            )
            return jsonify({"error": "Forbidden"}), 403
        ...
        error_logger.error(f"Agent error for model '{model_choice}': {e}", exc_info=True)
...
async def end_session():
    ...
    try:
        ...
        if username != current_user:
            security_logger.warning(
                f"Forbidden session deletion attempt by '{current_user}' on user '{username}'."
            )
            return jsonify({"error": "Forbidden"}), 403
        ...
        app_logger.info(f"Session '{session_name}' deleted for user '{username}'.")
